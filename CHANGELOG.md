...
- Trimmed the generic IP/FTO overlay so it only enables the add-on and no longer overrides environment-specific image selection.
...
- Moved the optional paid IP/FTO add-on overlay from `examples/preflight-values.ipfto.yaml` to `examples/addons/ipfto-values.yaml`.
